    ...
    entries: {
      weave: {
        ...
        config: {
          ...
          // Reads WANDB_API_KEY from env if wandbApiKey omitted.
...
1. `wandbApiKey: { source: "env", provider: "default", id: "WANDB_API_KEY" }` SecretRef
2. `wandbApiKey: { source: "file", provider: "default", id: "/path/to/key" }` SecretRef
3. `wandbApiKey: "<literal>"` (discouraged)
4. `process.env.WANDB_API_KEY` if `wandbApiKey` is omitted
