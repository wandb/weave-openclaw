...
describe("resolveConfig", () => {
  it("resolves apiKey + authSource (literal, env SecretRef; undefined when unset)", async () => {
    const literal = await resolveConfig({ project: "p", apiKey: "literal-key" }, ctx());
    ...
    const unset = await resolveConfig({ project: "p" }, ctx());
  ...
  it("requires project and builds projectId (entity/project, or bare when entity unset)", async () => {
  ...
  it("applies field defaults (captureContent, agentVersion, flush clamp)", async () => {
    ...
    expect((await resolveConfig({ project: "p", flushIntervalMs: 200 }, ctx())).flushIntervalMs).toBe(1000);
  });
});
