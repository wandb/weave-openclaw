 ...
 * 3. `process.env.WANDB_API_KEY` if `wandbApiKey` is undefined.
 *
...
export async function resolveWandbApiKey(
  wandbApiKey: string | SecretRef | undefined,
): Promise<ResolvedWandbAuth> {
  const ref = coerceSecretRef(wandbApiKey);
  if (ref) {
    return await resolveSecretRef(ref);
  }
  if (typeof wandbApiKey === "string") {
    const trimmed = wandbApiKey.trim();
    if (trimmed.length === 0) {
      throw new Error(
        `weave.wandbApiKey is an empty string. Set a non-empty W&B API key or omit to read ${ENV_FALLBACK_KEY} from env.`,
      );
    }
  ...
  throw new Error(
    `weave: no API key configured. Set weave.wandbApiKey in plugin config or export ${ENV_FALLBACK_KEY}.`,
  );
}
