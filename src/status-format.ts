...
export function formatWeaveStatus(
  ...
  if (snap.config) {
    ...
    lines.push(`endpoint: ${c.endpoint}`);
    ...
    lines.push(
      `flushIntervalMs: ${c.flushIntervalMs}  emitGenAiAliases: ${c.emitGenAiAliases}  stripSenderWrapper: ${c.stripSenderWrapper}`,
    );
    lines.push(`captureContent: ${c.captureSummary}`);
  }

  if (snap.exportStats) {
    const s = snap.exportStats;
    lines.push(
      `exports: ${s.exportsSucceeded} ok, ${s.exportsFailed} failed, ${s.spansExported} spans sent`,
    );
    if (s.lastSuccessAt !== undefined) {
      lines.push(`lastSuccess: ${formatRelative(s.lastSuccessAt, now)}`);
    } else {
      lines.push("lastSuccess: never");
    }
    if (s.lastFailureAt !== undefined) {
      const msg = s.lastFailureMessage ?? "<no message>";
      lines.push(`lastFailure: ${formatRelative(s.lastFailureAt, now)} — ${msg}`);
      if (s.lastFailureHint) {
        lines.push(`  hint: ${s.lastFailureHint}`);
      }
    }
  } else if (snap.lifecycle === "running") {
    // Running with no exporter stats means a test injected its own exporter —
    // skip the line rather than printing zeroes that look like "broken".
    lines.push("exports: (no observer wired)");
  }
