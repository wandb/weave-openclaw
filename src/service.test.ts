...
// Shared in-memory exporter that the mocked `weave.getWeaveTracer` returns
// spans into. Tests read `_testExporter.getFinishedSpans()` after acting,
// and call `_testExporter.reset()` between tests.
const _testExporter = new InMemorySpanExporter();
const _testProvider = new BasicTracerProvider({
  spanProcessors: [new SimpleSpanProcessor(_testExporter)],
});
...
vi.mock("weave", () => ({
  ...
  flushOTel: vi.fn(async () => {
    await _testExporter.forceFlush?.();
  }),
  getWeaveTracer: (name: string) => _testProvider.getTracer(name),
}));
...
describe("createWeaveService (integration)", () => {
  ...
  test("emits invoke_agent with chat and execute_tool as siblings for a full turn", async () => {
    ...
    const spans = _testExporter.getFinishedSpans();
    ...
    expect(invoke.attributes["gen_ai.conversation.id"]).toBe("conv-abc");
    expect(chat.attributes["gen_ai.conversation.id"]).toBe("conv-abc");
    expect(tool.attributes["gen_ai.conversation.id"]).toBe("conv-abc");

    // Agent identity propagates everywhere.
    expect(invoke.attributes["gen_ai.agent.name"]).toBe("test-agent");
    expect(chat.attributes["gen_ai.agent.name"]).toBe("test-agent");
    expect(tool.attributes["gen_ai.agent.name"]).toBe("test-agent");
    expect(invoke.attributes["weave.agent.version"]).toBe("v1.0");

    // Token usage on chat.
    expect(chat.attributes["gen_ai.usage.input_tokens"]).toBe(150);
    expect(chat.attributes["gen_ai.usage.output_tokens"]).toBe(40);

    // Tool identity.
    expect(tool.attributes["gen_ai.tool.name"]).toBe("search_web");
    expect(tool.attributes["gen_ai.tool.call.id"]).toBe("call_xyz");
  ...
  test("drops untrusted events (security boundary)", async () => {
    ...
    await flushAsyncDiagnostics();
    await service.stop?.(ctx);
    expect(_testExporter.getFinishedSpans()).toHaveLength(0);
  });
  ...
  test("error on a model call sets ERROR status with error.type", async () => {
    ...
    const chat = _testExporter.getFinishedSpans().find((s) => s.name.startsWith("chat"));
  ...
  test("tool.loop event becomes a span event on the active invoke_agent", async () => {
    ...
    expect(loopEvent?.attributes?.["gen_ai.tool.name"]).toBe("search_web");
  });
  ...
  test("startSubagentSpan creates a child invoke_agent parented under requester", async () => {
    ...
    expect(child?.attributes["gen_ai.conversation.id"]).toBe("conv-sub");
  ...
  test("invoke_agent is allowed even when its claimed parent is not active (legitimate root)", async () => {
    ...
    expect(_testExporter.getFinishedSpans()).toHaveLength(1);
    expect(_testExporter.getFinishedSpans()[0].name).toBe("invoke_agent x");
  });
