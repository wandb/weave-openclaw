...
/**
 * Static `weave.*` → `gen_ai.*` alias table for dual-emit. Mirrors the
 * server-side `_ALIAS_TO_CANONICAL` map at
 * https://github.com/wandb/weave/blob/master/weave/trace_server/agents/semconv.py
 * (each `Attribute(..., gen_ai_alias="gen_ai.*")` declaration). Only entries
 * that have a documented alias are listed — Weave-only extensions like
 * `weave.compaction.*`, `weave.reasoning_content`, `weave.cost.usd`,
 * `weave.context.*`, `weave.subagent.*`, `weave.session.*`,
 * `weave.message.*`, `weave.harness.*`, `weave.failure.*`, `weave.outcome`,
 * `weave.latency.*`, `weave.run.*`, `weave.tool.denied_reason`,
 * `weave.tool.block.reason` are intentionally not aliased.
 *
 * `weave.system` is also intentionally absent — it's the deprecated form of
 * `weave.provider.name`; we never emit it.
 */
const WEAVE_TO_GEN_AI: Readonly<Record<string, string>> = Object.freeze({
  "weave.operation.name": "gen_ai.operation.name",
  "weave.provider.name": "gen_ai.provider.name",
  "weave.agent.name": "gen_ai.agent.name",
  "weave.agent.id": "gen_ai.agent.id",
  "weave.agent.description": "gen_ai.agent.description",
  "weave.agent.version": "gen_ai.agent.version",
  "weave.request.model": "gen_ai.request.model",
  "weave.response.model": "gen_ai.response.model",
  "weave.response.id": "gen_ai.response.id",
  "weave.response.finish_reasons": "gen_ai.response.finish_reasons",
  "weave.usage.input_tokens": "gen_ai.usage.input_tokens",
  "weave.usage.output_tokens": "gen_ai.usage.output_tokens",
  "weave.usage.reasoning_tokens": "gen_ai.usage.reasoning_tokens",
  "weave.usage.cache_creation.input_tokens":
    "gen_ai.usage.cache_creation.input_tokens",
  "weave.usage.cache_read.input_tokens": "gen_ai.usage.cache_read.input_tokens",
  "weave.conversation.id": "gen_ai.conversation.id",
  "weave.conversation.name": "gen_ai.conversation.name",
  "weave.tool.name": "gen_ai.tool.name",
  "weave.tool.type": "gen_ai.tool.type",
  "weave.tool.call.id": "gen_ai.tool.call.id",
  "weave.tool.description": "gen_ai.tool.description",
  "weave.tool.definitions": "gen_ai.tool.definitions",
  "weave.tool.call.arguments": "gen_ai.tool.call.arguments",
  "weave.tool.call.result": "gen_ai.tool.call.result",
  "weave.request.temperature": "gen_ai.request.temperature",
  "weave.request.max_tokens": "gen_ai.request.max_tokens",
  "weave.request.top_p": "gen_ai.request.top_p",
  "weave.request.top_k": "gen_ai.request.top_k",
  "weave.request.frequency_penalty": "gen_ai.request.frequency_penalty",
  "weave.request.presence_penalty": "gen_ai.request.presence_penalty",
  "weave.request.seed": "gen_ai.request.seed",
  "weave.request.stop_sequences": "gen_ai.request.stop_sequences",
  "weave.request.choice.count": "gen_ai.request.choice.count",
  "weave.output.type": "gen_ai.output.type",
  "weave.input.messages": "gen_ai.input.messages",
  "weave.output.messages": "gen_ai.output.messages",
  "weave.system_instructions": "gen_ai.system_instructions",
});

/**
 * Mutate `attrs` by adding a `gen_ai.*` alias for every aliased `weave.*`
 * key. Skips Weave-only extensions (no alias mapping). Idempotent.
 *
 * Special cases:
 *   - `weave.operation.name=context_compacted` is NOT aliased: that value
 *     is not a member of OTel's `gen_ai.operation.name` enum and would
 *     pollute the namespace. Other operation values (chat, invoke_agent,
 *     execute_tool) are valid OTel operation names and are aliased.
 *
 * Exported for tests.
 *
 * @internal
 */
export function applyGenAiAliases(attrs: WeaveAttrs): void {
  for (const [weaveKey, genAiKey] of Object.entries(WEAVE_TO_GEN_AI)) {
    if (!(weaveKey in attrs)) continue;
    if (genAiKey in attrs) continue;
    const value = attrs[weaveKey];
    if (value === undefined) continue;
    if (
      weaveKey === "weave.operation.name" &&
      value === "context_compacted"
    ) {
      continue;
    }
    attrs[genAiKey] = value;
  }
}
...
export function mapDiagnosticEventToWeaveSpan(
  ...
  if (cfg.emitGenAiAliases && (result.kind === "start" || result.kind === "finalize")) {
    applyGenAiAliases(result.attrs);
  }
  return result;
}
...
function finalizeChat(
  ...
  if (usageFromHook) {
    assignInt(attrs, "weave.usage.input_tokens", usageFromHook.input);
    assignInt(attrs, "weave.usage.output_tokens", usageFromHook.output);
    assignInt(attrs, "weave.usage.cache_read.input_tokens", usageFromHook.cacheRead);
    assignInt(attrs, "weave.usage.cache_creation.input_tokens", usageFromHook.cacheWrite);
    assignInt(attrs, "weave.usage.reasoning_tokens", usageFromHook.reasoning);
  } else {
    const usage = (e.usage ?? null) as Record<string, unknown> | null;
    if (usage) {
      assignInt(attrs, "weave.usage.input_tokens", usage.input);
      assignInt(attrs, "weave.usage.output_tokens", usage.output);
      assignInt(attrs, "weave.usage.cache_read.input_tokens", usage.cacheRead);
      assignInt(attrs, "weave.usage.cache_creation.input_tokens", usage.cacheWrite);
      assignInt(attrs, "weave.usage.reasoning_tokens", usage.reasoning);
    } else {
      assignInt(attrs, "weave.usage.input_tokens", e.inputTokens);
      assignInt(attrs, "weave.usage.output_tokens", e.outputTokens);
    }
  }
  ...
  if (responseModel) attrs["weave.response.model"] = responseModel;
  ...
  if (finishReason) {
    ...
    if (arr) attrs["weave.response.finish_reasons"] = arr;
  }
  ...
  if (outputType) attrs["weave.output.type"] = outputType;
...
function startTool(
  ...
  const attrs: WeaveAttrs = {
    ...
    "weave.tool.name": toolName,
    "weave.tool.type": asString(e.toolType) ?? "function",
    ...agentAttrs(cfg, e),
    ...conversationAttrs(conversationId, e),
    ...providerAttrs(e),
  };
  if (callId) attrs["weave.tool.call.id"] = callId;
...
function conversationAttrs(
  ...
  const out: WeaveAttrs = {};
  if (conversationId) out["weave.conversation.id"] = conversationId;
...
function providerAttrs(e: Record<string, unknown>): WeaveAttrs {
  // We deliberately use `weave.provider.name`, NOT the deprecated alias
  // `weave.system` (`weave/trace_server/agents/semconv.py:72-74`). Don't
  // backfill `weave.system` as a duplicate — it'll be removed upstream.
  const provider = asString(e.provider);
  return provider ? { "weave.provider.name": provider } : {};
}

function modelAttrs(e: Record<string, unknown>): WeaveAttrs {
  const model = asString(e.model);
  return model ? { "weave.request.model": model } : {};
}
